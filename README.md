...
Drop PDFs in `data/user-uploads/` (override with `FORMIQO_USER_UPLOADS_DIR`). After `POST /api/v1/user-uploads/process-once` or `python scripts/process_user_uploads.py`, files are moved to `processed/` or `failed/` as **`{original_filename_stem}_{job_id}.pdf`** (same pattern for successes and normal pipeline failures; rare duplicate archive names get `_2`, `_3`, … before the extension). Rare failures outside normal intake (for example an unexpected exception after a successful run) still archive under `failed/` using that pattern, with a freshly generated UUID as `job_id` in the filename when needed.
